...
class Game:
    def __init__(self, k, n, q, players):
        ...
        self.player_losses = {i: 0 for i in range(q)}  # keep track of how many losses each player (play position) has
    ...
    def play(self):
        ...
        for config in self.configs:
            ...
            if config.win():
                ...
                losers = self.players[:].remove(winner)
                self.player_losses.update(
                    {i: self.player_losses[i] + 1 for i in range(self.q) if i != (self.turn - 1 % self.q)}
                )
                if winner:
                    self.player_wins[self.turn - 1] += 1
                    winner.reward()
        ...
        if self.configs:  # if there remain any configurations to expand, then play another turn
            ...
            return "Exhausted all configurations"
